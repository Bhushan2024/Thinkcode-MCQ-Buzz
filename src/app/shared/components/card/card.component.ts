...
export class CardComponent implements OnInit, AfterContentInit, BlockableUI {
  ...
  @Output() cardClickedEvent = new EventEmitter<string>();
  ...
  @ContentChildren(ProjectionTemplate) templates: QueryList<ProjectionTemplate> | undefined;
  ...
  ngOnInit() {
    ...
    this.navigateTo = this.navigateTo || this?.configProperties?.navigateTo || '';
    this.elementFormControlName =
      this.elementFormControlName || this?.configProperties?.formControlName || '';
    this.styleClass = this.styleClass || this?.configProperties?.styles?.stylesClassName || '';
  }
