...
import { faSearch, faFileCsv, faPlus, faFloppyDisk } from '@fortawesome/free-solid-svg-icons';
...
export class DataGridComponent implements OnInit, AfterViewInit {
  ...
  setGridOptions() {
    if (this.gridIterfaceConfig) {
      ...
      this.defaultColDef = {
        headerClass: this.configData.styles.datagridHeaderStyle,
        ...
        flex: 1,
        minWidth: 100,
        cellClass: this.configData.styles.datagridCellStyle,
      };
  ...
  setColumnDefs(gridIterfaceConfig: any) {
    ...
    this.colDefs = gridIterfaceConfig.columns.map((c: any) => ({
      ...
      headerName: c.label,
      cellStyle: {
        textAlign: c.alignment === 'left' ? 'left' : c.alignment === 'right' ? 'right' : 'center',
        padding: 10,
      },
      ...
      cellRenderer: (data: any) => {
        if (c.type === 'dateTime') {
          ...
          return data?.value ? `<span class="clickable-txt w-100">${data.value}</span>` : '';
        } else {
          return data.value;
        }
      },
  ...
  getDropDownOptions(params: any) {
    ...
    if (colDef?.dynamicDropdownOptions) {
      ...
      if (listItemsString) {
        ...
        if (listItems) {
          const dropdownOptions = listItems.map((list: any) => list[colDef.optionsKey]);
  ...
  isCellEditable(cell: any) {
    if (cell.data.isNewRow && cell.colDef.isColumnEditable) {
      ...
      (this.gridIterfaceConfig.isRowEditable || this.gridIterfaceConfig.isBulkEditEnabled)
    ) {
      return true;
    } else {
      return false;
  ...
  handleCellClick(cellObj: any) {
    const clickedCol = this.colDefs.find((item) => item.field === cellObj?.colDef?.field);
    if (clickedCol) {
  ...
  onClickAddRow() {
    ...
    if (this.gridIterfaceConfig?.isBulkAddEnabled) {
      ...
      if (this.gridIterfaceConfig.isRowAddable && !this.gridIterfaceConfig.isRowEditable) {
        this.editType = 'fullRow';
        setTimeout(() => {
  ...
  setActionColumn() {
    if (this.gridIterfaceConfig.isRowEditable && !this.gridIterfaceConfig.isEditDrawerEnabled)
      this.actionColumnDefs.push({
        ...
        cellRenderer: (data: any) => {
          return this.gridIterfaceConfig.isRowEditable
            ? `<button class="btn btn-sm edit-row-btn">
    ...
    if (this.gridIterfaceConfig.isRowDeletable && !this.gridIterfaceConfig.isEditDrawerEnabled)
      this.actionColumnDefs.push({
        headerName: '',
        headerClass: 'text-center',
        cellClass: '',
        ...
        cellRenderer: (data: any) => {
          return this.gridIterfaceConfig.isRowDeletable
            ? `<button class="btn btn-sm delete-row-btn">
                  <svg role="img" aria-hidden="true" focusable="false" data-prefix="fas" data-icon="trash" class="svg-inline--fa fa-trash fa-w-14" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">
  ...
  onRowValueChanged($event: any) {
    ...
    if (
      (this.gridIterfaceConfig?.isRowEditable && !this.gridIterfaceConfig?.isBulkEditEnabled) ||
      (this.gridIterfaceConfig?.isRowAddable && !this.gridIterfaceConfig?.isRowEditable)
    ) {
      if (this.gridIterfaceConfig?.isRowAddable && !this.gridIterfaceConfig?.isRowEditable) {
        this.editType = '';
      }
  ...
  onCellValueChanged($event: any) {
    ...
    if (this.gridIterfaceConfig.isBulkEditEnabled || this.gridIterfaceConfig.isBulkAddEnabled) {
      this.editedRowsIndexes.push($event.rowIndex);
    }
  }
  ...
  saveRowData($event: any) {
    ...
    filteredData = this.gridRowData
      .filter((row: any, index: number) => this.editedRowsIndexes.includes(index))
      .sort((a: any, b: any) => {
    ...
    this.gridRowData = this.gridRowData.filter(
      (obj: any) =>
        !Object.entries(obj).every(([key, value]) => key === 'isNewRow' || value === null),
    );
    const dataToEmit = {
  ...
  onRowClicked(rowValue: any) {
    ...
    if (this.gridIterfaceConfig?.isEditDrawerEnabled && this.allowOpeningDrawer) {
      const initialState = {
  ...
  ngAfterViewInit(): void {
    this.applyPaginationPanelStyles();
  }

  applyPaginationPanelStyles(): void {
    const paginationPanel = this.el.nativeElement.querySelector('.ag-paging-panel');
    if (paginationPanel) {
      const classArray = this.configData.styles.datagridPaginationStyle.split(' ');
      classArray.forEach((className: string) => paginationPanel.classList.add(className));
    }
  }
}
